...
def get_ssl_context(server_side: bool, ciphersuites: str = None) -> Optional[SSLContext]:
    ...
    if server_side:
        try:
            update_secc_cert_chain()  # required because SECC_LEAF could've changed at runtime
...
def update_secc_cert_chain():
    with open(os.path.join(get_PKI_PATH(), CertPath.CPO_CERT_CHAIN_PEM), 'w') as outfile:
        with open(os.path.join(get_PKI_PATH(), CertPath.SECC_LEAF_PEM), 'r') as secc_leaf:
            _secc_leaf = secc_leaf.read()
            outfile.write(_secc_leaf)
            outfile.write("\n")
        with open(os.path.join(get_PKI_PATH(), CertPath.CPO_SUB_CA2_PEM), 'r') as cpo_sub_ca2:
            _cpo_sub_ca2 = cpo_sub_ca2.read()
            outfile.write(_cpo_sub_ca2)
            outfile.write("\n")
        with open(os.path.join(get_PKI_PATH(), CertPath.CPO_SUB_CA1_PEM), 'r') as cpo_sub_ca1:
            _cpo_sub_ca1 = cpo_sub_ca1.read()
            outfile.write(_cpo_sub_ca1)
